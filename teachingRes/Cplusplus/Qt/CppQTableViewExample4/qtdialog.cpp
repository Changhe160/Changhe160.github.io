...
struct MyModel : public QStandardItemModel
{
      ...
      m_data(MyDataTable())
  {
    ...
    const int n_rows = static_cast<int>(m_data.data.size());
    ...
    if (!(this->rowCount() == static_cast<int>(m_data.data.size())))
    {
      std::cout << this->rowCount() << '\n';
    }
    assert(this->rowCount() == static_cast<int>(m_data.data.size()));
  }
  ...
  QVariant data(const QModelIndex &index, int role) const
  {
    ...
    if (!(index.row() < static_cast<int>(m_data.data.size())))
    {
      std::cout << index.row();
    }
    assert(index.row() < static_cast<int>(m_data.data.size()));
